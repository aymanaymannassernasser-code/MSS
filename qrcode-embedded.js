...
// Minimal implementation for Motor Starter Simulator
// Based on QRCode.js but simplified and self-contained
...
(function(window) {
    ...
    const QRCodeGenerator = {
        generate: function(text, canvas, options) {
            options = options || {};
            const size = options.width || 200;
            const margin = options.margin || 2;
            
            try {
                // Use qrcodegen library (embedded below)
        ...
        toCanvas: function(canvas, text, options) {
            return this.generate(text, canvas, options);
        }
    };
    
    // Minimal QR Code generation algorithm (compact version)
            ...
            for (let version = minVersion; version <= maxVersion; version++) {
                ...
                if (usedBits <= dataCapacityBits) {
                    ...
                    for (const seg of segs) {
                        appendBits(seg.mode.modeBits, 4, dataCodewords);
                        appendBits(seg.numChars, seg.mode.numCharCountBits(version), dataCodewords);
                        for (const b of seg.getData())
                            dataCodewords.push(b);
                    }
                    
                    const dataCapacityCodewords = Math.floor(dataCapacityBits / 8);
                    appendBits(0, Math.min(4, dataCapacityBits - dataCodewords.length), dataCodewords);
                    appendBits(0, (8 - dataCodewords.length % 8) % 8, dataCodewords);
                    
                    for (let padByte = 0xEC; dataCodewords.length < dataCapacityCodewords; padByte ^= 0xEC ^ 0x11)
                        appendBits(padByte, 8, dataCodewords);
                    
                    const bb = [];
                    for (let i = 0; i < dataCodewords.length; i += 8) {
                        let byte = 0;
                        for (let j = 0; j < 8; j++)
                            byte = byte << 1 | dataCodewords[i + j];
                        bb.push(byte);
                    }
    ...
    console.log('✅ Embedded QR Code generator loaded');
